...
var io = {
	...
	open: function () {
      ...
      notify.remove();
		...
		io.socket.onmessage = function (event) {
			...
			console.log("[io.fhem] receiving data: " + event.data);
			switch (data.cmd) {
        case 'reloadPage':
          ...
          break;
				case 'item': // item
					for (var i = 0; i < data.items.length; i = i + 2) {
						item = data.items[i];
						val = data.items[i + 1];
						console.log("[io.fhem]: update item: " + item + " val: " + val);
						widget.update(item, val);
					}
					;
					/*
		...
		io.socket.onerror = function (error) {
			notify.error('Driver: fhem', 'Websocket error: ' + error);
		};
		io.socket.onclose = function () {
      io.close();
      notify.add('ConnectionLost', 'warning', "Driver: fhem", "Connection to the fhem server lost!");
      notify.display();
		};
	},
	...
	send: function (data) {
		if (io.socket.readyState == 1) {
			...
			console.log('[io.fhem] sending data: ' + JSON.stringify(data));
			// } else {
			// notify.error('Driver: fhem', 'Websocket not available. Could not send data!');
		}
	},
